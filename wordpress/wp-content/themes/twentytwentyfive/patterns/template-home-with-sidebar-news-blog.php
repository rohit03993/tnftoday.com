					...
					<div class="tnf-hero-grid">
						...
						<div class="tnf-hero-side">
							...
							if ( $latest->have_posts() ) :
								echo '<ul class="tnf-hero-side__list">';
								$rank = 0;
								while ( $latest->have_posts() ) :
									$latest->the_post();
									++$rank;
									echo '<li><a class="tnf-hero-headline" href="' . esc_url( get_permalink() ) . '">';
									echo '<span class="tnf-hero-headline__rank" aria-hidden="true">' . (int) $rank . '</span>';
									echo '<span class="tnf-hero-headline__thumb"><img src="' . esc_url( twentytwentyfive_tnf_news_thumbnail_url( (int) get_the_ID() ) ) . '" alt="" loading="lazy" decoding="async" /></span>';
									echo '<span class="tnf-hero-headline__body"><span class="tnf-hero-headline__title">' . esc_html( get_the_title() ) . '</span>';
									echo '<time class="tnf-hero-headline__date" datetime="' . esc_attr( get_the_date( DATE_W3C ) ) . '">' . esc_html( get_the_date() ) . '</time></span>';
									echo '</a></li>';
								endwhile;
								echo '</ul>';
								wp_reset_postdata();
